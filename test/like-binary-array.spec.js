...
describe('Bitz mimic Array', () => {
  ...
  it('.filter should work exactly like Array.prototype.filter', () => {
    ...
    expect(array1.filter(isPrime)).toEqual(
      Bitz.from(array1).filter(isPrime).items
    )

    let arrayInvalidEntries = 0
    let bitzyArrayInvalidEntries = 0
    const array2 = [
      { id: 15 },
      { id: -1 },
      { id: 0 },
    ...
    expect(
      ...
      Bitz.from(array2).filter(item => {
        if (Number.isFinite(item.id) && item.id !== 0) return true
        bitzyArrayInvalidEntries++
        return false
      }).items
    )
    expect(arrayInvalidEntries).toEqual(bitzyArrayInvalidEntries)

    const fruits = ['apple', 'banana', 'grapes', 'mango', 'orange']
    const bitzyFruits = Bitz.from(fruits)

    const filterItems = (arr, query) =>
      arr.filter(el => el.toLowerCase().includes(query.toLowerCase()))
    expect(filterItems(fruits, 'ap')).toEqual(
      filterItems(bitzyFruits, 'ap').items
    ) // ['apple', 'grapes']
    expect(filterItems(fruits, 'an')).toEqual(
      filterItems(bitzyFruits, 'an').items
    ) // ['banana', 'mango', 'orange']
  })
  ...
  it('operations 1 .map, .filter, .mergeSort, .reverse, .slice, .reduce, .flat should modify the collection the same', () => {
    ...
    expect([...rasultBitzyArray]).toEqual(resultArray)
    expect(resultArray.length).toEqual(rasultBitzyArray.length)

    const flatBinArr = rasultBitzyArray.concat(
      Bitz.from([
        51,
        12,
        33,
        Bitz.from([
          Bitz.from([1, 2, 3, 4, 5, Bitz.from([1, 2, 3, 4]), 1]),
          Bitz.from([2, 3, 4, 5]),
          Bitz.from([23, Bitz.from([222, 33, 1, 2])]),
        ]),
      ]).flat(3)
    )
    ...
    const infiniteBinNest = Bitz.from([
      Bitz.from([
        Bitz.from([Bitz.from([1, 2, 3, 4]), Bitz.from([1, 2, 3, 4]), 3, 4]),
        Bitz.from([1, Bitz.from([1, 2, Bitz.from([1, 2, 3, 4]), 4]), 3, 4]),
      ]),
      Bitz.from([Bitz.from([1, 2, 3, 4]), Bitz.from([1, 2, 3, 4])]),
    ])
  ...
  it('.flat, flatten should work the same way', () => {
    const arr = [4, 1, 1, 2, 3, 8, 7]
    const binArr = Bitz.from(arr)
    const rasultBitzyArray = binArr.map(i => i ** i + 10 - 2)
      ...
      Bitz.from([
        ...
        Bitz.from([
          Bitz.from([
            ...
            5,
            rasultBitzyArray,
            [...Bitz.from([1, 2, 3, 4]).reverse().splice(1, 2)],
            1,
          ]),
          Bitz.from([2, 3, 4, 5]).slice(2, 4),
          Bitz.from([23, Bitz.from([222, 33, 1, 2])]),
        ]),
      ])
        .reverse()
    ...
    const flatMapBitzyArray = Bitz.from([
      Bitz.from([1, 2, 3, 4]),
      Bitz.from([1, 2, 3, 4]).reverse(),
    ])

    expect(flatMapBitzyArray.flatten(x => x * 10 + 4).items).toEqual(
      flatMapArray.flatMap(item => item.map(x => x * 10 + 4))
    )
    expect([1, 2, [3, 4]].flat()).toEqual(
      new Bitz().with(1, 2, new Bitz().with(3, 4)).flat().items
    )

    expect([1, 2, [3, 4, [5, 6]]].flat()).toEqual(
      new Bitz().with(1, 2, new Bitz().with(3, 4, new Bitz().with(5, 6))).flat()
        .items
    )
    expect([1, 2, [3, 4, [5, 6]]].flat(2)).toEqual(
      new Bitz()
        .with(1, 2, new Bitz().with(3, 4, new Bitz().with(5, 6)))
        .flat(2).items
    )
      ...
      new Bitz()
        .with(
          ...
          new Bitz().with(
            3,
            4,
            new Bitz().with(5, 6, new Bitz().with(7, 8, new Bitz().with(9, 10)))
          )
        )
        .flat(Infinity).items
    )
  ...
  it('.splice should modify the array in place', () => {
    ...
    const binMonths = Bitz.from(months)
  ...
  it('.includes should return expected array', () => {
    ...
    expect([1, 2, 3].includes(2)).toBe(Bitz.from([1, 2, 3]).includes(2))
    expect([1, 2, 3].includes(4)).toBe(Bitz.from([1, 2, 3]).includes(4))
    expect([1, 2, 3].includes(3, 3)).toBe(Bitz.from([1, 2, 3]).includes(3, 3))
    expect([1, 2, 3].includes(3, -1)).toBe(Bitz.from([1, 2, 3]).includes(3, -1))
    expect([1, 2, NaN].includes(NaN)).toBe(Bitz.from([1, 2, NaN]).includes(NaN))
    expect(['1', '2', '3'].includes(3)).toBe(
      Bitz.from(['1', '2', '3']).includes(3)
    )
  })
  ...
  it('.find should return expected array', () => {
    ...
    const lastBin1 = Bitz.from(lastArr1)
    const lastBin2 = Bitz.from(lastArr2)
  ...
  it('.every should work like array.every', () => {
    ...
    expect(Bitz.from([12, 5, 8, 130, 44]).every(isBigEnough)).toBe(
      [12, 5, 8, 130, 44].every(isBigEnough)
    )
    expect(Bitz.from([12, 54, 18, 130, 44]).every(isBigEnough)).toBe(
    ...
    expect(
      isSubset(Bitz.from([1, 2, 3, 4, 5, 6, 7]), Bitz.from([5, 7, 6]))
    ).toBe(isSubset([1, 2, 3, 4, 5, 6, 7], [5, 7, 6])) // true
    expect(
      isSubset(Bitz.from([1, 2, 3, 4, 5, 6, 7]), Bitz.from([5, 8, 7]))
    ).toBe(isSubset([1, 2, 3, 4, 5, 6, 7], [5, 8, 7])) // false

    expect(Bitz.from([1, undefined, 3]).every(x => x !== undefined)).toBe(
      [1, undefined, 3].every(x => x !== undefined)
    ) // true
    expect(Bitz.from([2, undefined, 3]).every(x => x === 2)).toBe(
    ...
    const bitz = Bitz.from([1, 2, 3, 4])
    let out2 = ''
    bitz.every((elem, index, arr) => {
    ...
    const bitz2 = Bitz.from([1, 2, 3])
    let out4 = ''
    bitz2.every((elem, index, arr) => {
      arr.push('new')
    ...
    const bitz3 = Bitz.from([1, 2, 3, 4])
    let out6 = ''
    bitz3.every((elem, index, arr) => {
      arr.pop()
  ...
  expect([2, 5, 8, 1, 4].some(isBiggerThan10)).toBe(
    Bitz.from([2, 5, 8, 1, 4]).some(isBiggerThan10)
  ) // false
  expect([12, 5, 8, 1, 4].some(isBiggerThan10)).toBe(
    Bitz.from([12, 5, 8, 1, 4]).some(isBiggerThan10)
  ) // true
  expect([2, 5, 8, 1, 4].some(x => x > 10)).toBe(
    Bitz.from([2, 5, 8, 1, 4]).some(x => x > 10)
  ) // false
  expect([12, 5, 8, 1, 4].some(x => x > 10)).toBe(
    Bitz.from([12, 5, 8, 1, 4]).some(x => x > 10)
  ) // true
})
